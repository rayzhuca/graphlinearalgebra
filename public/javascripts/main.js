...
space.add({
  animate: (time, ftime) => {
    ...
    graph.draw(
      currBound,
      vectors.values(),
      previousMatrix.interpolateBounded(
        transformMatrix,
        easeInOut((time - offsetTime) / 1000)
      )
    );
  },
...
function pointermove_handler(ev) {
  ...
  if (evCache.length == 2) {
    ...
    document.getElementById("log").innerHTML =
      Math.round((prevDiff - curDiff) * 100) / 100 / 100;
